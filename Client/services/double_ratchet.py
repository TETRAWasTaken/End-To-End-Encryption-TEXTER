...
class DoubleRatchetSession:
    ...
    def DHRatchet_for_bob_intial(self, spk_private_key: x25519.X25519PrivateKey, partner_dh_pub_obj: x25519.X25519PublicKey):
        """
        Performs the initial DH ratchet step for the responder (Bob).
        This uses our Signed Pre-Key and the partner's DH public key.
        """
    ...
    def RatchetDecrypt(self, header: dict, body: dict) -> bytes:
        ...
        if header["dh_pub"] != self.DHr_b64:
            # This is a new ratchet key, but check if we've already advanced past the previous chain length
            if header["pn"] < self.PN:
                self.counters.increment('old_messages_discarded')
                raise InvalidTag("Received message from an old, discarded ratchet chain.")
