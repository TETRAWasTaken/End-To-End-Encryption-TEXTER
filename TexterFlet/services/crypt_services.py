...
from services import x3dh, utils
from services.double_ratchet import DoubleRatchetSession, bytes_to_b64str, b64str_to_bytes
...
from services.utils_path import get_app_data_dir
...
class CryptServices:
    def __init__(self, username: str) -> None:
        ...
        self.partner_bundles: dict[str, dict[str, Any]] = {}
    ...
    def _X3DH_KDF(self, km: bytes) -> bytes:
        ...
        salt = b'\x00' * 32
        ikm = F + km
        hkdf = HKDF(algorithm=hashes.SHA256(), length=32, salt=salt, info=b"X3DH", backend=default_backend())
        return hkdf.derive(ikm)

    def generate_and_save_key(self, password: str, overwrite: bool = False) -> dict[str, Any] | None:
        ...
        private_keys: utils.PrivateKeys = self.x3dh.get_private_keys_for_saving()
    ...
    def save_contacts_to_disk(self) -> None:
        if self.db.is_connected():
            self.db.save_contacts(list(self.sessions.keys()))

    def load_contacts_from_disk(self) -> list[str]:
        return self.db.get_all_contacts() if self.db.is_connected() else []

    def serializable_key_bundle(self, bundle: dict[str, Any]) -> dict[str, Any]:
        return {
            "identity_key": bytes_to_b64str(
                bundle["identity_key"].public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)),
            "identity_key_dh": bytes_to_b64str(
                bundle["identity_key_dh"].public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)),
            "signed_pre_key": bytes_to_b64str(
                bundle["signed_pre_key"].public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)),
            "signature": bytes_to_b64str(bundle["signed_pre_key_signature"]),
            "one_time_pre_keys": {
                str(key_id): bytes_to_b64str(
                    opk.public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw))
                for key_id, opk in bundle["one_time_pre_keys"].items()
            }
        }

    def store_partner_bundle(self, partner_username: str, bundle_json: dict[str, Any]) -> None:
        try:
            ik_str, ik_dh_str, spk_str, sig_str = (bundle_json.get(k) for k in
                                                   ["identity_key", "identity_key_dh", "signed_pre_key", "signature"])
            if not isinstance(ik_str, str) or not isinstance(ik_dh_str, str) or not isinstance(spk_str, str) or not isinstance(sig_str, str):
            ...
            opk_str = bundle_json.get("one_time_pre_key")
            opk_bytes = b64str_to_bytes(opk_str) if isinstance(opk_str, str) else None
            ...
            print(f"Error storing/verifying bundle: {e}")

    def _initiate_session_alice(self, partner_username: str) -> tuple[dict[str, Any], bytes]:
        if partner_username not in self.partner_bundles: raise Exception(f"No bundle for {partner_username}")
        bundle = self.partner_bundles[partner_username]
        p_ik_dh, p_spk_pub, p_opk_pub, opk_id = bundle['identity_key_dh'], bundle['signed_pre_key'], bundle.get(
            "one_time_pre_key"), bundle.get("one_time_key_id")

        ek_priv, ek_pub = self.utils.generate_x25519_key_pair()
        ik_priv = self.x3dh.identity_key_dh_private
        if ik_priv is None:
            raise ValueError("Identity DH private key is not loaded.")
        ...
        x3dh_header = {
            "ik_a": bytes_to_b64str(
                ik_priv.public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)),
            "ek_a": bytes_to_b64str(ek_pub.public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)),
            "opk_id": opk_id
        }
        return x3dh_header, SK

    def _initiate_session_bob(self, partner_username: str, x3dh_header: dict[str, Any], dr_header: dict[str, Any]) -> tuple[
        DoubleRatchetSession, bytes]:
        p_ik_str, p_ek_str, opk_id = x3dh_header.get('ik_a'), x3dh_header.get('ek_a'), x3dh_header.get('opk_id')
        if not isinstance(p_ik_str, str) or not isinstance(p_ek_str, str):
        ...
        if ik_priv is None or spk_priv is None:
            raise ValueError("Private keys are not loaded.")
        ...
        dh_pub_str = dr_header.get("dh_pub")
        if not isinstance(dh_pub_str, str):
            raise ValueError("DR header missing dh_pub or invalid type")
        alice_ratchet_pub = x25519.X25519PublicKey.from_public_bytes(b64str_to_bytes(dh_pub_str))
    ...
    def encrypt_message(self, partner_username: str, plaintext: str) -> dict[str, Any]:
    ...
    def decrypt_message(self, partner_username: str, payload: dict[str, Any]) -> str | None:
        x3dh_header = payload.get("x3dh_header")
        dr_header = payload.get("dr_header")
        dr_body = payload.get("dr_body")

        if not isinstance(dr_header, dict) or not isinstance(dr_body, dict):
            print("Invalid payload format.")
            return None

        try:
            if partner_username not in self.sessions:
                if not isinstance(x3dh_header, dict): raise Exception("Received message without valid x3dh_header")
                if partner_username not in self.partner_bundles: return "NEEDS_BUNDLE"
