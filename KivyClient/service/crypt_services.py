...
from KivyClient.service import x3dh, utils
from KivyClient.service.double_ratchet import DoubleRatchetSession, bytes_to_b64str, b64str_to_bytes
from KivyClient.service.database_service import DatabaseService
...
class CryptServices:
    def __init__(self, username: str):
        ...
        # Modified to use Kivy App user_data_dir
    ...
    def store_partner_bundle(self, partner_username: str, bundle_json: dict):
        try:
            ...
            ik_bytes, spk_bytes, signature_bytes = b64str_to_bytes(ik_str), b64str_to_bytes(spk_str), b64str_to_bytes(
                sig_str)
            opk_bytes = b64str_to_bytes(bundle_json.get("one_time_pre_key")) if bundle_json.get(
                "one_time_pre_key") else None
    ...
    def _initiate_session_bob(self, partner_username: str, x3dh_header: dict, dr_header: dict) -> tuple[
        ...
        SK = self._X3DH_KDF(km)
        dr = DoubleRatchetSession(SK)
        alice_ratchet_pub = x25519.X25519PublicKey.from_public_bytes(b64str_to_bytes(dr_header.get("dh_pub")))
    ...
    def decrypt_message(self, partner_username: str, payload: dict) -> str | None:
        ...
        try:
            if partner_username not in self.sessions:
                if not x3dh_header: raise Exception("Received message without session or x3dh_header")
